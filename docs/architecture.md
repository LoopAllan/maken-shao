...
- 結局資料以證據可支持的終盤路線為單位；每個詳細頁直接顯示觸發流程、內容脈絡、社群標籤與官方名稱界線。未完成旗標映射的社群標籤不得當成官方結局名稱。
- Boss 不另設重複頁面或導覽入口；角色圖鑑的 Boss 標籤是唯一圖鑑入口，靜態頁產生器亦不得重新加入 Boss 導覽。
- 手機版主選單展開後固定在 sticky header 下方，使用視窗內獨立垂直捲動，不隨文件頂端離開畫面。

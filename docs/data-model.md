...
| `isPlaceholder` | 可為 boolean 或 `null`；placeholder 必含固定警語。`spoiler`／`spoilerLevel` 已完全移除。 |

## 型別特有欄位

- `world.json`：`worldCategory`（v0.2 世界觀、版本脈絡與術語）
- `systems.json`：`systemArea`（v0.2 系統資料與研究中機制）
- `characters.json`：`nameZhHant`、`nameJa`、`nameEn`、`nameEnStatus`、`imagePath`、圖片 provenance、`role`、`characterType`、`characterGroup`、`wikiNavigationGroup`、`wikiLinkedCharacterIds`、`tags`、`affiliations`、`brainJackStatus`、`firstAppearanceWalkthroughId`、`relatedWalkthroughIds`
...
- `walkthrough.json`：`sequence`、`area`、`routeId`、`routeTitle`、`objectives`、`prerequisites`、`anyOfPrerequisites`、`branchChoiceIds`、`bossIds`、`brainJackTargetIds`、`missable`
  ...
  - 所有資料型別均已移除 `spoiler` 與 `spoilerLevel`；標題、摘要、區域、條件與正文由前端直接渲染。
  - `characterGroup` 提供互斥主分類 tabs；`tags` 提供可複選且採 AND 語意的次級查詢。`communityReferences` 記錄指定 Wiki 角色頁 revision；圖片欄位分開記錄原始 Wiki 資產（MIME、尺寸、bytes、SHA-1）、實際最大 640px WebP 下載 URL、本地 SHA-256、File 頁、上傳資訊、內容類型、原始資產重抓驗證狀態及明確的未知 artist/source/license 狀態。
  - `wikiNavigationGroup` 保存 `Template:MX` 的 Playable／Non-playable 原始分組；`wikiLinkedCharacterIds` 只收頁面正文的明示 wikilink，不計共用導航模板注入的全站連結。

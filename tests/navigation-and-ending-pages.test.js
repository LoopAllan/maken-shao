...
test('keeps an opened mobile navigation fixed and independently scrollable beneath the sticky header', async () => {
  ...
  assert.match(css, /\.sidebar\.is-open \{[^}]*position:fixed;[^}]*top:var\(--mobile-menu-top\);[^}]*height:calc\(100dvh - var\(--mobile-menu-top\)\);[^}]*overflow-x:hidden;[^}]*overflow-y:scroll;[^}]*overscroll-behavior-y:contain;[^}]*touch-action:pan-y;/);
});
